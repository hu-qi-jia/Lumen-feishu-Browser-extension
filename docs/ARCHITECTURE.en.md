...
| Outbound lockdown | `config.ts isFeishuOutboundAllowed` + `providers.ts assertSafeBaseUrl` + CSP | Only allows connecting to Feishu + the LLM |
...
| App Secret | `appSecret.ts` | Two tiers: plaintext / password-encrypted |
...
- **App Secret two tiers** (pick one at build time): plaintext `VITE_FEISHU_APP_SECRET` (bundled) / password-encrypted
...
For all build-time variables (across the two deployments: personal / store) see [`.env.example`](../.env.example)
and [`PROJECT.en.md`](PROJECT.en.md) §9 — including the App Secret two tiers,
