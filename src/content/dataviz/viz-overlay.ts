...
interface Overlay {
  ...
  /** Slides data when this overlay renders a PPT deck (for HTML export). */
  slides?: Slide[]
  deckName?: string
  deckTheme?: 'light' | 'dark'
}
...
function ensure(id: string): Overlay {
  ...
  // Title-bar actions live on the page (the sandbox can't draw chrome). The print button asks the
  // iframe to print itself → the user's print dialog → "Save as PDF" captures the full page incl. charts.
  const actions = document.createElement('div')
  ...
  const print = iconBtn('打印', '打印 / 导出 PDF（含图表整页）')
  print.onclick = () => overlays.get(id)?.iframe.contentWindow?.postMessage({ type: 'DATAVIZ_PRINT' }, '*')
  const exportHtml = iconBtn('导出', '导出为 HTML 文件')
  exportHtml.style.display = 'none' // only shown for slide decks
  exportHtml.onclick = () => {
    const o = overlays.get(id)
    if (o?.slides) downloadSlidesHtml(o.slides, o.deckName || '演示文稿', getTheme(o.deckTheme === 'dark' ? 'night' : 'business'))
  }
  const close = iconBtn('关闭', '关闭')
  close.onclick = () => destroy(id)
  actions.append(submit, accent, exportHtml, print, close)
  ...
  const o: Overlay = { el, iframe, titleEl, submitBtn: submit, exportBtn: exportHtml, nonce: newNonce(), ready: false, pending: null }
  overlays.set(id, o)
  return o
}
...
/** Detect whether a payload renders a slide deck; return its slides for HTML export. */
function detectSlides(p: Payload): Slide[] | null {
  const spec = p.spec as { kind?: string; slides?: Slide[] } | undefined
  if (spec?.kind === 'slides') return spec.slides ?? (Array.isArray(p.data) ? (p.data as Slide[]) : [])
  if (p.code && /ui\.slides/.test(p.code)) return Array.isArray(p.data) ? (p.data as Slide[]) : []
  return null
}

/** Print (→ Save as PDF) the overlay's sandbox doc — used by SlidesPanel's 导出 PDF. */
export function printViz(vizId: string) {
  overlays.get(vizId)?.iframe.contentWindow?.postMessage({ type: 'DATAVIZ_PRINT' }, '*')
}
...
export function renderViz(p: Payload) {
  ...
  // Slides-only: surface the 导出 HTML button when this overlay shows a PPT deck.
  const slides = detectSlides(p)
  o.slides = slides ?? undefined
  o.deckName = p.name
  o.deckTheme = p.theme
  o.exportBtn.style.display = slides ? '' : 'none'
