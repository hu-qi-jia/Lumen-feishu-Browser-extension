...
export default function SmartFillPanel({ settings, context, disabled, onBack }: Props) {
  ...
  return (
    <div className="scenario-panel view-enter" key="smartfill">
      <TopBar title="智能填充" onBack={onBack} />
      <div className="sf-body">
        ...
        {onSupported && (
          <>
            ...
            <Button variant="primary" block onClick={preview} disabled={disabled || busy || loading || !target}>
              {busy && !plan ? '处理中…' : '预览填充'}
            </Button>
          </>
        )}
        ...
        {plan && plan.proposed.length > 0 && (
          <>
            ...
            <Button variant="primary" block onClick={apply} disabled={busy}>
