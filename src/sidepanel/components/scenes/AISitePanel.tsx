...
export default function AISitePanel({ settings, context, disabled, onBack }: Props) {
  ...
  return (
    <div className="scenario-panel view-enter" key="aisite">
      <TopBar title="AI 建站" onBack={onBack} />
      <div className="as-body">
        ...
        {!hasGen ? (
          <div className="as-row">
            <Button onClick={genPlan} disabled={disabled || busy || !onFeishuTable || !request.trim()}>生成方案</Button>
            <Button variant="primary" onClick={() => generate(false)} disabled={disabled || busy || !onFeishuTable || !request.trim()}>直接生成</Button>
          </div>
        ) : (
          <Button variant="primary" block onClick={() => generate(false)} disabled={disabled || busy || !request.trim()}>重新生成</Button>
        )}
        ...
        {planText && (
          <div className="as-plan">
            ...
            <Button variant="primary" block onClick={() => generate(false)} disabled={busy}>按方案生成网站</Button>
          </div>
        )}

        {hasGen && (
          <Button block onClick={() => generate(true)} disabled={disabled || busy || !request.trim()}>按上面文字微调（只改你说的那处）</Button>
        )}
        {canSave && <Button block onClick={save} disabled={busy}>保存为「我的网站」</Button>}
        {hasGen && <Button block onClick={newDraft} disabled={busy}>新建一个</Button>}

        {hasGen && (
          <div className="as-export">
            <div className="as-section">输出</div>
            <Button block onClick={exportDoc} disabled={disabled || busy}>导出飞书文档</Button>
            ...
            {report && (
              <div className="as-report">
                ...
                <Button variant="primary" block onClick={() => openUrl(report.url)}>打开文档 →</Button>
                <div className="as-row">
                  ...
                  <Button onClick={pushToChat} disabled={busy || !chatId}>发到群</Button>
                </div>
