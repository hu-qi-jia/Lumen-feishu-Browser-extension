...
type FeishuKind = 'base' | 'sheet' | 'doc' | 'wiki'
...
// wikiToken ?? appToken ?? spreadsheetToken ?? documentId).
function resourceToken(f: ReturnType<typeof parseFeishuContext>): string | null {
  if (!f) return null
  return f.wikiToken ?? f.appToken ?? f.spreadsheetToken ?? f.documentId ?? null
}
...
// glance in the dropdown. Wiki wraps another type (usually a doc) → doc icon.
