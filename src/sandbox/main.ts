...
const ui = {
   ...
   * slide to page, a dot strip to jump, and a print-stack so the overlay's print button exports ALL pages as PDF.
   */
  slides(el: HTMLElement, list: SlideSpec[], rows?: Rows): void {
      ...
      '<div class="slides-stage" tabindex="0">' +
        ...
        '<div class="slides-bar">' +
          ...
          '<div class="slides-nav">' +
            ...
            '<button class="page slides-theme" title="切换深色">切换深色</button>' +
            '<button class="page slides-next">›</button>' +
          '</div>' +
        '</div>' +
      '</div>' +
      // Built on-demand at print time (one slide per page) so print exports the whole deck.
    ...
    const updTheme = () => { themeBtn.textContent = isDark() ? '切换浅色' : '切换深色'; themeBtn.title = isDark() ? '切换浅色' : '切换深色' }
    ...
    // beforeprint/afterprint cover Ctrl+P; the print button path builds explicitly (see DATAVIZ_PRINT).
